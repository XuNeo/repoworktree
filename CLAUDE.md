...
- **Child repo under parent worktree** (`layout.py`): When a parent repo is a worktree, child repos (separate git repos) are NOT included in the git checkout. `_build_level` uses `inside_worktree` flag to create intermediate dirs and symlink child repos on top. `_exclude_child_repos` hides these overlaid symlinks from `git status` by writing patterns to the worktree's per-worktree `info/exclude` file via `extensions.worktreeConfig` + `core.excludesFile`.
