...
- **Child repo under parent worktree** (`layout.py`): When a parent repo is a worktree, child repos (separate git repos) are NOT included in the git checkout. `_build_level` uses `inside_worktree` flag to create intermediate dirs and symlink child repos on top. `_exclude_child_repos` hides these from git via **sparse-checkout** (excludes child repo paths from the working tree entirely) and `.gitignore` (hides the symlinks as untracked entries). This ensures all standard git commands (`git status`, `git diff`) work transparently — no files are hidden from the user. Requires Git ≥ 2.34 for per-worktree sparse-checkout support.
