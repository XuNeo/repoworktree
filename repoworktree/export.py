...
def export(
    ...
    for wt in meta.worktrees:
        ...
        # Count commits
        count_result = _git(
            ["rev-list", "--count", f"{src_head}..HEAD"],
            cwd=wt_path,
        )
        ...
        if fmt == "patch":
            result = _export_patch(
                wt_path, wt.path, safe_name, src_head, output_dir, commit_count
            )
        elif fmt == "bundle":
